...
#define WIFI_SSID       "Pratama Center-2.4G-ext"//"BRT Juken"
#define WIFI_PASS       "gladagan2004"//"A1b2c3d4e5"
#define TAG "ESP32-CAM"
#define LED_GPIO        GPIO_NUM_14

#define BACKEND_URL     "http://leafiot.ksmiotupnvj.com:8000/chili/upload"    
#define DHT_PIN 4
#define DEVICE_NAME "esp32cam-01"
#define DHT_URL "http://leafiot.ksmiotupnvj.com:8000/sensor/dht"  
...
void dht_http_task(void *pvParameters)
{
    while (1) {
        float temperature = 0;
        float humidity = 0;
        ...
        // --- Baca sensor DHT22 ---
        if (dht22_read(DHT_PIN, &temperature, &humidity) == ESP_OK) {
            ...
            if (err == ESP_OK) {
                ESP_LOGI(TAG, "HTTP POST Success | Status: %d",
            ...
            ESP_LOGW(TAG, "Gagal membaca sensor DHT22");
        }
...
void app_main(void) {
    ...
    xTaskCreate(dht_http_task, "dht_task", 4096, NULL, 5, NULL);

    
    // Wajib: cegah app_main() keluar
    while (1) {
        vTaskDelay(pdMS_TO_TICKS(1000));
    }
}
